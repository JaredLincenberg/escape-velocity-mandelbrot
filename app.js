(function () {
  ...
  // Puts every strand back at its z2 starting pose, in place (no rebuild).
  function resetSwirlToStart() {
    if (!swirl) return;
    var cx = swirl.cx, cy = swirl.cy, zx = swirl.zx, zy = swirl.zy;
    var step = swirl.step, prevDir = swirl.prevDir, curDir = swirl.curDir;
    for (var k = 0; k < swirl.count; k++) {
      var z2 = mandelStep(cx[k], cy[k], cx[k], cy[k]);
      zx[k] = z2.x; zy[k] = z2.y;
      step[k] = 2;
      var dx = z2.x - cx[k], dy = z2.y - cy[k];
      var len = Math.sqrt(dx * dx + dy * dy) || 1;
      curDir[k * 2] = dx / len; curDir[k * 2 + 1] = dy / len;
      prevDir[k * 2] = curDir[k * 2]; prevDir[k * 2 + 1] = curDir[k * 2 + 1];
    }
  ...
  // Manual scrub for the orbit-iteration slider. There's no per-point
  // history stored (only the previous/current direction), so seeking --
  // including backward -- replays from the z2 starting pose; replaying up
  // to MAX_ITER steps for every strand is still only ~count*200 complex
  // multiply-adds, negligible.
  function seekSwirlTo(target) {
    if (!swirl) return;
    target = Math.max(2, Math.min(MAX_ITER, Math.round(target)));
    resetSwirlToStart();
    for (var i = 0; i < target - 2; i++) advanceSwirlStep();
    swirlIterFloat = target;
    swirl.lastStepped = target;
    material.uniforms.uSwirlFrac.value = 0;
    syncSwirlIterUI();
  }

  function syncSwirlIterUI() {
    var slider = document.getElementById('ctl-swirl-iter');
    var label = document.getElementById('swirl-iter-label');
    var val = String(Math.round(swirlIterFloat));
  ...
  // Every slider's starting value -- Reset Values reverts to these, but
  // leaves toggles, the color scheme, and layer visibility untouched.
  var SLIDER_DEFAULTS = {
    density: 170, cre: -0.75, cim: 0, span: 3.5, zoom: BASE_RADIUS,
    'spin-speed': 0.05, 'swirl-speed': 1
  };
  ...
  function resetValues() {
    Object.keys(SLIDER_DEFAULTS).forEach(function (id) {
      var range = document.getElementById('ctl-' + id);
      var num = document.getElementById('ctl-' + id + '-val');
      var v = SLIDER_DEFAULTS[id];
      if (range) range.value = String(v);
      if (num) num.value = String(v);
    });

    params.density = SLIDER_DEFAULTS.density;
    params.centerRe = SLIDER_DEFAULTS.cre;
    params.centerIm = SLIDER_DEFAULTS.cim;
    params.span = SLIDER_DEFAULTS.span;
    spin.speed = SLIDER_DEFAULTS['spin-speed'];
    swirlSpeed = SLIDER_DEFAULTS['swirl-speed'];
    material.uniforms.uBands.value = 8;
    var bandsNum = document.getElementById('ctl-bands-val');
    var bandsRange = document.getElementById('ctl-bands');
    if (bandsRange) bandsRange.value = '8';
    if (bandsNum) bandsNum.value = '8';

    tween = null;
    orbit.radius = SLIDER_DEFAULTS.zoom;
    updateCameraFromOrbit();
    syncZoomUI();

    buildField(); // also re-inits swirl, resetting the iteration slider to 2
    buildFlatField();
  }
  ...
  function wireUI() {
    ...
    function pauseSwirlAuto() {
      swirlEnabled = false;
      if (swirlToggle) { swirlToggle.textContent = 'STILL'; swirlToggle.classList.remove('is-active'); }
    }
    if (swirlToggle) {
    ...
    var swirlIterSlider = document.getElementById('ctl-swirl-iter');
    var swirlIterMinus = document.getElementById('swirl-iter-minus');
    var swirlIterPlus = document.getElementById('swirl-iter-plus');
    if (swirlIterSlider) {
      swirlIterSlider.addEventListener('input', function () {
        pauseSwirlAuto();
        seekSwirlTo(parseInt(swirlIterSlider.value, 10));
      });
    }
    if (swirlIterMinus) {
      swirlIterMinus.addEventListener('click', function () {
        pauseSwirlAuto();
        seekSwirlTo(Math.round(swirlIterFloat) - 1);
      });
    }
    if (swirlIterPlus) {
      swirlIterPlus.addEventListener('click', function () {
        pauseSwirlAuto();
        seekSwirlTo(Math.round(swirlIterFloat) + 1);
      });
    }
    ...
    var resetValuesBtn = document.getElementById('reset-values-btn');
    if (resetValuesBtn) resetValuesBtn.addEventListener('click', resetValues);
  ...
  function tick() {
    ...
    if (swirlEnabled) {
      ...
      swirlIterFloat = Math.min(MAX_ITER, swirlIterFloat + swirlDt * swirlSpeed);
      var guard = 0;
